...
async function synchronizeBggGames() {
    ...
    await mongoose.connect(
        ...
        console.log('Oops, something went wrong, this is the error log ==> ' + err);
    });
    console.log('Counting your games...');
    let gameList = undefined;
    await axios.get(`http://localhost:57492/collection/${process.env.BGG_USER}`)
            .then((result) => {
                gameList = result.data;
            })                
            .catch((err) => {
                console.log("\x1b[41m" + "Whoops, something went wrong: " + err.request.res.statusCode + ' ' + err.request.res.statusMessage + "\x1b[0m");
            });
    ...
    for (let gameItem of gameList) {
        ...
        if(!result) {
            await axios.get(`http://localhost:57492/thing/${gameItem.gameId}`)
                .then(async (result) => {
                ...
                .catch((err) => {
                    console.log("\x1b[41m" + "Whoops, something went wrong: " + err + "\x1b[0m");
                    console.log(`Error info: id => ${gameItem.gameId} | name => ${gameItem.name}`);
                });
                await sleep(5000);
        }else{
